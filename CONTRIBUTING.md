# Contributing to AllStak .NET SDK
...
```bash
dotnet restore src/AllStak/AllStak.csproj
```
...
```bash
dotnet build src/AllStak/AllStak.csproj --configuration Release --no-restore\ndotnet test tests/AllStak.Tests/AllStak.Tests.csproj --configuration Release --verbosity normal
```
